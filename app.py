...
<div style="background:linear-gradient(135deg,#0d1b2a,#1a2744);border:1px solid #1e3a5f;border-radius:8px;padding:18px 28px;margin-bottom:20px;">
  <div style="font-size:20px;font-weight:700;color:#4fc3f7;letter-spacing:2px;">
    🛡️ 涉诈智能研判与决策支持系统 v3.1
  </div>
  <div style="font-size:15px;color:#546e7a;margin-top:4px;font-family:'Share Tech Mono',monospace;">
    OSINT-BASED FRAUD INTELLIGENCE SYSTEM | 涉诈网站研判 · 招聘诈骗识别 · 信息库 · 反诈预警 | 陕西警察学院信息技术系&侦查系联合开发
  </div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    st.markdown("### ⚙️ 分析参数")
    analyst_id = st.text_input("警号", placeholder="如：24D084", value=st.session_state.get("analyst_id",""))
    ...
    st.markdown("---\n### 🤖 AI 引擎")
    _engine_opts = {"仅 DeepSeek": "deepseek"}
...
with tab_url:
    ...
    if analyze_url_btn and url_input.strip():
            ...
            if any(feat.keyword_hits.values()):
                ...
                for level, words in feat.keyword_hits.items():
                    if words:
                        ...
                        # ==============================================
            # 🎯 Pyecharts 风险雷达图（完美适配你的系统）
            # ==============================================
            st.markdown("---")
            st.markdown("### 🎯 风险维度雷达图（Pyecharts）")

            # 取出你的 5 大风险维度分数
            score_map = wras.score_breakdown
            # 读取风险数据
            score_map = wras.score_breakdown

            # 👇 这个格式是 pyecharts 强制要求的格式
            data = [
                [
                    score_map.get("network",15),
                    score_map.get("content",30),
                    score_map.get("fraud",20),
                    score_map.get("sentiment", 10),
                    score_map.get("blacklist", 10),
                ]
            ]

            # 👇 这个 schema 是标准写法，绝对不报错
            c = (
                Radar()
                .add_schema(
                    schema=[
        ...
        with t4:
            st.markdown("### 🤖 AI 智能分析报告（DeepSeek）")

            # ===================== AI 报告展示框 =====================
            if report.gemini and report.gemini.ai_report:
                with st.container():
                    st.markdown(f"""
                    <div style="background:#f0f7ff; padding:20px; border-radius:12px; border-left:5px solid #1a73e8; line-height:1.8; font-size:15px; color:#111;">
                    {report.gemini.ai_report}
                    </div>
                    """, unsafe_allow_html=True)

                # ===================== 邮件发送状态 =====================
                if report.wras.risk_level.value in ["RED", "ORANGE"]:
                    st.success("✅ 已自动向高校就业办发送预警邮件！")
                else:
                    st.info("ℹ️ 风险等级不足，未发送邮件")

            else:
                st.warning("⚠️ AI 报告未生成 → 正在使用规则引擎分析，不影响风险评分")
                if report.wras.risk_level.value in ["RED", "ORANGE"]:
                    st.success("✅ 已自动向高校就业办发送预警邮件！")
                else:
                    st.info("ℹ️ 风险等级不足，未发送邮件")
...
# ==============================================================
# 🗨️ 聊天记录反诈分析（最强版 · 支持文字 + 图片上传 + AI识别）
# ==============================================================
st.markdown("## 🗨️ 聊天记录反诈检测")
st.markdown("支持粘贴文字 / **上传聊天截图** → AI自动识别诈骗")

tab_chat_text, tab_chat_img = st.tabs(["📝 粘贴聊天文字", "🖼️ 上传聊天截图"])

with tab_chat_text:
    chat_input = st.text_area(
        "粘贴完整聊天记录",
        height=220,
        placeholder="粘贴微信/QQ/招聘聊天记录…"
    )
    analyze_chat_btn = st.button("🔍 分析聊天记录", type="primary", use_container_width=True)

with tab_chat_img:
    chat_img = st.file_uploader("上传聊天截图（自动识别文字）", type=["png", "jpg", "jpeg"])
    analyze_chat_img_btn = st.button("🚀 图片AI反诈分析", type="primary", use_container_width=True)

# ==========================
# 文字聊天分析逻辑
# ==========================
if analyze_chat_btn and chat_input.strip():
    with st.spinner("🤖 正在AI深度分析聊天记录…"):
        from backend.modules.ai_vision_processor import ai_analysis
        result = ai_analysis(chat_input.strip())

        st.markdown("### 📝 聊天内容")
        st.code(chat_input.strip())

        st.markdown("### 🤖 AI反诈深度分析")
        st.markdown(f"""
<div style="background:#1A2C42; padding:20px; border-radius:12px; color:white; line-height:1.8;">
{result}
</div>
""", unsafe_allow_html=True)
...
# 图片聊天分析逻辑（最强OCR + AI）
# ==========================
if analyze_chat_img_btn and chat_img is not None:
    with st.spinner("🔍 图片识别中 → 提取文字 → AI分析…"):
        # 保存图片
        with open("temp_chat.png", "wb") as f:
            f.write(chat_img.getbuffer())

        # 最强识别
        from backend.modules.ai_vision_processor import super_image_analyze
        res = super_image_analyze("temp_chat.png")

        st.image(chat_img, use_column_width=True)

        st.markdown("### 📝 截图识别文字")
        st.text_area("", res["extract_text"], height=200)

        st.markdown("### 🤖 AI反诈分析结果")
        st.markdown(f"""
<div style="background:#1A2C42; padding:20px; border-radius:12px; color:white; line-height:1.8;">
{res["ai_analysis"]}
</div>
...
# ==========================
with tab_chat_img:
    chat_img = st.file_uploader("上传聊天截图", type=["png","jpg","jpeg"])
    analyze_chat_img_btn = st.button("🚀 分析截图", type="primary")

if analyze_chat_img_btn and chat_img is not None:
    with st.spinner("🔍 正在处理图片..."):
        with open(r"img\\temp_chat.png", "wb") as f:
            f.write(chat_img.getbuffer())

        from backend.modules.ai_vision_processor import super_image_analyze
        res = super_image_analyze(r"img\\temp_chat.png")

        # ======================
        # ✅ 弹出文字框（可复制、可检查）
        # ======================
        st.subheader("📝 图片识别文字（可复制）")
